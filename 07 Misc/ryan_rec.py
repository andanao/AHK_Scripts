...
class ArticleDB:
    ...
    def article_read(self, art_id,user):
        ...
        for i, val in enumerate(self.df.columns[:-1]): #should get the # or tags, probably should have a better way if more things are going to be added to the DB
            if self.df.loc[art_id][i]:
                # print(val)
                user.likes[val] = 1 #set to one increment later
...
if __name__ == "__main__":
    ...
    df = db.df
    user = db.users[0]
    db.article_read(df.index[0],db.users[0])